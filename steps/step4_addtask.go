...
func AddTaskWM(w http.ResponseWriter, r *http.Request) {
	...
	if parseDate.Before(time.Now()) {
		if task.Repeat == "" || task.Repeat == " " || task.Date == time.Now().Format("20060102") {
			...
			if err != nil {
				w.WriteHeader(http.StatusInternalServerError)
				writeInfo(w, Task{Error: "Oшибка функции вычисления даты выполнения задачи"})
				return
			}
		}
	} else {
	...
	insertId, err := Insert(DBConn, task)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		writeInfo(w, Task{Error: "Oшибка функции добавления записи в БД"})
		return
	}
